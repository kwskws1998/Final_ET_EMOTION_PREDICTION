...
DEFAULT_RUN_DIR = Path("artifacts/trt_only_roberta")
DEFAULT_OUTPUT_DIR = Path("hf_emotion_trt_roberta")
DEFAULT_ZIP_PATH = Path("hf_emotion_trt_roberta_upload.zip")
DEFAULT_WEIGHT_NAME = "emotion_trt_predictor_seed42.safetensors"
...
def build_metric_row(metrics_path: Path, run_dir: Path, lr_label: str) -> dict[str, str]:
    ...
    selected_score = payload.get("selected_score", valid_mae.get("TRT", valid_mae.get("all")))
    row = {
        "lr": lr_label,
...
def find_source_weight(run_dir: Path, requested_weight_name: str) -> Path | None:
    candidates = [
        run_dir / requested_weight_name,
        *sorted(run_dir.glob("emotion_trt_predictor_seed*.safetensors")),
        *sorted(run_dir.glob("trt_predictor_seed*.safetensors")),
        *sorted(run_dir.glob("et_predictor2_seed*.safetensors")),
    ]
    seen: set[Path] = set()
...
def copy_or_write_manifest(run_dir: Path, output_dir: Path, weight_name: str, best_row: dict[str, str]) -> None:
    ...
    lines = [
        ...
        "PRETRAIN_CSV=data/final_training/final_pretrain_trt_scaled.csv",
        "FINETUNE_CSV=data/final_training/final_finetune_trt_scaled.csv",
    ]
    (output_dir / "manifest.env").write_text("\n".join(lines) + "\n", encoding="utf-8")


def metric_table_for_output(best_row: dict[str, str], output_dim: int) -> str:
    if output_dim == 1:
        return (
            "| Feature | MAE |\n"
            "|---|---:|\n"
            f"| TRT | {read_metric_for_readme(best_row, 'TRT')} |"
        )
    rows = [
        "| Feature | MAE |",
        "|---|---:|",
        f"| nFix | {read_metric_for_readme(best_row, 'nFix')} |",
        f"| FFD | {read_metric_for_readme(best_row, 'FFD')} |",
        f"| GPT | {read_metric_for_readme(best_row, 'GPT')} |",
        f"| TRT | {read_metric_for_readme(best_row, 'TRT')} |",
        f"| fixProp | {read_metric_for_readme(best_row, 'fixProp')} |",
        f"| all | {read_metric_for_readme(best_row, 'all')} |",
    ]
    return "\n".join(rows)


def write_readme(output_dir: Path, best_row: dict[str, str], weight_name: str, output_dim: int) -> None:
    best_lr = best_row.get("lr") or "n/a"
    head_description = "hidden size 768 -> 1" if output_dim == 1 else "hidden size 768 -> 5"
    feature_description = "`TRT`" if output_dim == 1 else "`[nFix, FFD, GPT, TRT, fixProp]`"
    trt_index_line = "- TRT index: `0`" if output_dim == 1 else "- TRT index: `3`"
    metric_table = metric_table_for_output(best_row, output_dim)
    title = "Emotion-specific TRT Predictor" if output_dim == 1 else "Emotion-specific ET Predictor 2"
    output_note = (
        "This export predicts one scalar per token: scaled total reading time (TRT)."
        if output_dim == 1
        else "This export predicts five CMCL-style ET features per token."
    )
    readme = f"""---
...
# {title}: Official ZuCo/Provo -> Emotion Gaze

This repository contains a RoBERTa-base token-level eye-tracking predictor. {output_note}

## Model

- Encoder: `roberta-base`
- Head: linear regression layer, {head_description}
- Output feature order: {feature_description}
{trt_index_line}
...
def package_hf_model(args: argparse.Namespace) -> None:
    ...
    output_dim = infer_output_dim(output_dir / args.weight_name)
    copy_model_wrapper(output_dir, args.weight_name)
    write_readme(output_dir, best_row, args.weight_name, output_dim)
