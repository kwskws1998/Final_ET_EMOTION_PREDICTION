"""TRT-only word-level datasets for emotion-specific ET prediction."""
...
def validate_trt_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    missing = {"sentence_id", "word_id", "word", TARGET_FEATURE} - set(df.columns)
    if missing:
        raise ValueError(f"Missing TRT columns: {sorted(missing)}")

    clean = df.copy()
    clean = clean.dropna(subset=["sentence_id", "word_id", "word", TARGET_FEATURE])
    clean["sentence_id"] = clean["sentence_id"].astype(int)
    clean["word_id"] = clean["word_id"].astype(int)
    clean["word"] = clean["word"].astype(str)
    clean[TARGET_FEATURE] = pd.to_numeric(clean[TARGET_FEATURE], errors="coerce")
    clean = clean.dropna(subset=[TARGET_FEATURE])
    clean = clean[clean["word"].map(clean_word).ne("")]
...
def renumber_sentences(df: pd.DataFrame, offset: int = 0) -> pd.DataFrame:
    clean = validate_trt_dataframe(df)
...
def load_and_concat_csvs(paths: Iterable[Path]) -> pd.DataFrame:
    ...
    offset = 0
    for path in paths:
        frame = renumber_sentences(pd.read_csv(path), offset=offset)
        frames.append(frame)
...
def split_by_sentence(
    ...
    train_df = renumber_sentences(clean[~clean["sentence_id"].isin(valid_ids)])
    valid_df = renumber_sentences(clean[clean["sentence_id"].isin(valid_ids)])
...
class SimpleVocab:
    ...
    def build(cls, dataframes: Iterable[pd.DataFrame], min_freq: int = 1) -> "SimpleVocab":
        ...
        for df in dataframes:
            for word in df["word"].map(clean_word):
...
class SimpleTRTDataset(torch.utils.data.Dataset):
    """Tiny whitespace-token backend for smoke tests without model downloads."""
    ...
    def _sentence_words(self, sentence_id: int) -> list[str]:
        ...
        return rows["word"].map(clean_word).tolist()

    def _compute_pad_to_length(self) -> int:
        lengths = [
            len(self.vocab.encode(self._sentence_words(sentence_id), self.max_length))
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        values_np = rows[TARGET_FEATURE].to_numpy(dtype=np.float32)
        input_ids = self.vocab.encode(words, max_length=self.max_length)
        attention_mask = [1] * len(input_ids)
        labels = np.full((len(input_ids),), IGNORE_VALUE, dtype=np.float32)
        n_assign = min(len(words), self.max_length - 2)
        labels[1 : 1 + n_assign] = values_np[:n_assign]
        return {
...
class HFTRTDataset(torch.utils.data.Dataset):
    """Hugging Face tokenizer dataset with first-subtoken word supervision."""
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        labels = np.full((len(encoded["input_ids"]),), IGNORE_VALUE, dtype=np.float32)
        seen: set[int] = set()
        for token_index, word_index in enumerate(word_ids):
            if word_index is None or word_index in seen or word_index >= len(values_np):
                continue
            labels[token_index] = values_np[word_index]
            seen.add(word_index)
        return {
...
def collate_trt_batch(
    ...
    labels = torch.full((batch_size, max_len), IGNORE_VALUE, dtype=torch.float32)
    for index, item in enumerate(batch):
        seq_len = item["input_ids"].shape[0]
        input_ids[index, :seq_len] = item["input_ids"]
        attention_mask[index, :seq_len] = item["attention_mask"]
        labels[index, :seq_len] = item["labels"]
